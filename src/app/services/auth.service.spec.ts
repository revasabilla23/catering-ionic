...
export class AuthService {
  ...
  login(credentials: { email: string, password: string }): Observable<any> {
    return this.http.post(`${environment.apiUrl}/auth/login`, credentials).pipe(
      switchMap((res: any) => {
        return from(this._storage!.set(TOKEN_KEY, res.access_token)).pipe(
          switchMap(() => this.http.get<any>(`${environment.apiUrl}/auth/me`))
        );
      }),
      tap(user => {
        // <-- LANGKAH DEBUGGING: Cetak objek user ke konsol
        console.log('Data Pengguna dari API:', user); 
    ...
    if (token) {
      this.http.get<any>(`${environment.apiUrl}/auth/me`).subscribe({
        next: (user) => {
          this.currentUser.next(user);
          this.isAuthenticated.next(true);
          // Pastikan 'user.role' ada sebelum redirect
          if (user && user.role) {
             this.redirectUser(user.role);
          } else {
             this.router.navigateByUrl('/home'); // Fallback jika tidak ada role
          }
        },
        error: () => {
          this.isAuthenticated.next(false);
  ...
  // <-- PERBAIKAN: Fungsi redirect dibuat lebih aman
  redirectUser(role: string) {
    // Ubah peran menjadi huruf kecil untuk menghindari masalah besar/kecil huruf
    ...
    if (userRole === 'hrga') {
      ...
      // Jika peran bukan 'hrga' atau 'karyawan', arahkan ke home
